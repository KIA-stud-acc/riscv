...
module riscv_ps(input   logic         clk, rst,
  ...
  logic[31:0] instrD, PCE, PCtargetE, targetImmExt;
  ...
  datapath    dp( .clk(clk), .rst(rst), .regWriteD(regWrite), .jumpD(jumpD), .branchD(branch), .ALUsrcBD(ALUsrcB), 
                  ...
                  .uo(uo), .ui(ui), .targetImmExt(targetImmExt));
  ...
  logic       corr_pred, bpPred, bpValid, bpUpdate, uo, ui;
  logic[31:0] bpDest;

  branch_predictor  bp( .adrr(PC), .adrw(PCE), .wd(targetImmExt), .we(bpUpdate), .clk(clk), .rst(rst), 
